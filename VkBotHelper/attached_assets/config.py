...
def init_supabase():
    ...
    if SUPABASE_URL and SUPABASE_KEY:
        try:
            ...
            # Проверяем наличие функции increment через тест-запрос
            try:
                supabase.rpc("increment", {"x": 0}).execute()
                logging.info("Функция increment существует в базе данных")
            except Exception as e:
                logging.warning(f"Функция increment не найдена: {e}")
                logging.info("Функция increment не найдена, но это не критично для работы приложения")
...
def init_telegram():
    ...
    # Телеграм клиент не нужен, так как мы используем aiogram
    telegram_client = None
